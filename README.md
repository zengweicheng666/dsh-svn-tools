...
- **历史**：log 列表；点击版本 → 上下分栏（上栏列表、下栏该版本日志与变更路径，分隔条可拖动调整比例）；点击文件路径 → 该版本与上一版本（rN vs rN-1）的只读左右对比（复用「变更」对比视图，中文/特殊字符路径走仓库 URL 编码读取）；文件行悬停显示两个回退按钮（均修改工作副本、不会自动提交，文件以 M/A/D 呈现在「提交」页，由用户自行提交；「修改前」= 该文件上一次变更的修订，即该文件被 rN 修改之前的版本，可能远早于 rN-1）：
