...
- **变更**：状态列表（徽标 + changelist 标签），**单击文件行=选中高亮**；**行悬停出现「比对」按钮（单击即查看 diff），双击行同样进入 diff**；**左键按下并拖动=框选多行**（框选只加高亮、不动勾选，松手后**点击组内任一行身、或勾选/取消组内任一行的复选框**，都会把该行的勾选状态同步给整组——组同步在单击后延迟约 0.25s 执行，双击进比对时自动取消，不会误翻转整组勾选）；**左右版本对比视图**（左列版本库 BASE、右列工作副本，等宽双栏 + 自动换行永不截断，红/绿高亮增删行，可切回文本 diff）；**差异导航**：对比视图顶部提供「◀ 上一处 / 下一处 ▶」（上一处差异 / 下一处差异）在差异块间跳转，自动滚动文本使选中差异块进入可视区（块可放下时居中）并高亮，附 `当前位置/总数` 计数；导航条吸顶固定，滚动时不会被顶出视野；**差异块操作**：悬停任意差异块可选择「采用左侧 / 采用右侧 / 都保留·左先 / 都保留·右先」直接修改工作副本文件；行内操作（中文按钮）：
  - `比对`：查看该文件差异（悬停行出现；双击行亦可）
  - `添加`（未版本化）、`删除`、`还原`（确认弹窗）
  - `追溯`：逐行追溯视图
  - `解决`（冲突 C 状态）：弹窗选择 我的版本 / 仓库版本 / 基准版本 / 保留当前
  - `忽略`（未版本化）：快捷写入 svn:ignore
- **提交**：勾选文件或提交全部、中文日志（`✨ AI 生成日志` 用当前模型自动生成）、提交后显示新 revision；**提交范围上下联动**：下方「提交全部变更」勾上→上方所有可见文件全部勾选、取消→全部取消勾选；在列表勾选任一文件自动转「按所选提交」，取消最后一个勾选后停在「未勾选任何文件」（提交禁用，不再自动跳回提交全部）；**AI 生成日志只分析提交范围内的文件**（提交全部=全部可见变更；只勾部分=仅这些文件；未勾选文件不进分析），范围内为空时按钮禁用；**新增「显示无版本控制的文件」选项**（默认勾选）：取消勾选后 `?` 状态文件从列表隐藏，并**排除出「提交全部变更」与 AI 分析**（不会自动 add/提交），`!` 缺失文件不受影响、仍自动删除；**自动处理构建产物替换**（未版本化 `?` 文件自动 `svn add`、缺失 `!` 文件自动 `svn delete`，提交全部时对所选目录一并生效，结果在提示栏显示）；**变更列表工具栏批量操作**：列表顶部工具栏右侧按勾选情况显示「删除 / 回退」两个按钮（与行内按钮同语义、作用于全部勾选文件，未版本化 `?` 与冲突 `C` 文件不计入；无勾选时不显示；单次确认 + 执行后清空勾选，避免残留勾选泄漏进提交范围）
- **历史**：log 列表（首屏最近 30 条；列表底部「加载更早的版本」按钮逐页追加更早记录直到 r1，追尽后按钮变为「已显示全部 N 条历史」）；点击版本 → 上下分栏（上栏列表、下栏该版本日志与变更路径，分隔条可拖动调整比例）；**悬停「比对」按钮单击，或双击文件路径** → 该版本与上一版本（rN vs rN-1）的只读左右对比（复用「变更」对比视图，中文/特殊字符路径走仓库 URL 编码读取）；详情文件列表**常显勾选框（无单选/多选切换，始终多选）**，单击文件行=选中高亮，左键拖动框选 + 点击组内任一行身或该行复选框可把整组勾选状态同步（与提交页列表交互一致）；**行悬停出现与提交页对齐的右侧按钮组**（比对 / 撤销 rN 改动 / 回退到修改前 / 退回到此版本，向右对齐；这些操作只修改工作副本、不会自动提交，文件会以 M/A/D 重新出现在「提交」页由你自行提交）：
  ...
  - **多选批量工具栏**：分隔条下方工具栏左侧**常显「全选/清空」**（文件行勾选框常显，无单选/多选切换；也可左键拖动框选，再勾选/点击组内任一行身把该行勾选状态同步给整组——与提交页列表一致）；右侧按勾选情况显示「撤销 rN 改动 / 回退到修改前 / 退回到此版本」批量按钮——无勾选时不显示；含 r1 行时撤销/回退不显示。批量操作为单次确认 + 逐文件串行执行（避免工作副本锁冲突），失败文件记录并继续，结束时汇总成功/失败；切换版本详情自动清空勾选
