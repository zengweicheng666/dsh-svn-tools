...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dsh-svn-block.cur{outline:1px solid rgba(88,166,255,.8);outline-offset:1px;border-radius:3px;background:rgba(88,166,255,.07)}",
    ...
    function SidesView(props) {
      ...
      var nav = function (delta) {
        ...
        setCurIdx(next);
        var el = changeRefs[next];
        var scroller = scrollRef;
        // Scroll ONLY the container(s) that own the overflow — never
        // scrollIntoView, which drags ancestor panels along and shoves the
        // nav row out of view. Prefer the inner .dsh-svn-sides; if it has no
        // overflow of its own, fall back to the nearest scrollable ancestor.
        if (el && scroller && scroller.scrollTop !== undefined) {
          if (scroller.scrollHeight <= scroller.clientHeight + 1) {
            var n = el.parentElement;
            while (n && n !== document.body) {
              if (n.scrollHeight > n.clientHeight + 1 && n.scrollTop !== undefined) { scroller = n; break; }
              n = n.parentElement;
            }
          }
          ...
          if (eRect.top < cRect.top || eRect.bottom > cRect.bottom) {
            var fits = eRect.height <= cRect.height;
            var target;
            if (fits) {
              // fits the viewport: center it so every selected line is visible
              target = scroller.scrollTop + (eRect.top - cRect.top) - (cRect.height - eRect.height) / 2;
            } else if (eRect.top >= cRect.top) {
              // taller than viewport, block below/at the fold: show its start
              target = scroller.scrollTop + (eRect.top - cRect.top);
            } else {
              // taller than viewport, block scrolled past: show its end
              target = scroller.scrollTop + (eRect.bottom - cRect.bottom);
            }
            ...
            catch (e2) { scroller.scrollTop = target; }
          }
        }
      };
      var changeNo = -1;
      var blockEls = blocks.map(function (b) {
        ...
        if (isChange) {
          changeNo++;
          if (changeNo === curNo) cls += " cur";
          ref = function (el) { changeRefs[changeNo] = el; };
        }
        var bar = isChange
      ...
      return h("div", { className: "dsh-svn-sideswrap" },
        ...
        navRow,
        h("div", { className: "dsh-svn-sides", ref: function (el) { scrollRef = el; } }, blockEls)
      );
    }
