...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      if (props.openRev !== null && props.openRev !== undefined) {
        ...
        if (entry) {
          ...
          var files = (entry.paths || []).map(function (p, idx) {
            ...
            var canRevert = clickable && entry.revision >= 2 && !!props.onRevertFile;
            ...
            return h("div", { className: "dsh-svn-histfile" + (clickable ? "" : " disabled"),
              key: p.path + "-" + idx,
    ...
    function SvnPanel(props) {
      ...
      // History: revert one repo file to its PREVIOUS VERSION — the content
      // it had before `revision` modified it (the file's last-changed
      // revision before rN, not rN-1 which may not have touched the file).
      // mode "undo" = reverse merge (keeps later changes, may conflict);
      // mode "restore" = exact previous-version content overwrite (discards
      // later changes and local edits). Working copy only — no commit.
      function revertHistoryFile(revision, repoRel, action, mode, prevRev) {
        ...
        call("history-revert", payload({ repoRel: repoRel, revision: revision, mode: mode })).then(function (v) {
          var gotPrev = v.prevRev !== undefined && v.prevRev !== null;
      ...
      function selectRev(r) {
        if (openRev === r) { setOpenRev(null); return; }
        setOpenRev(r);
        var entry = null;
      ...
      if (checkoutMode) {
        ...
        body = h(HistoryView, { entries: logs || [], openRev: openRev,
          ...
          onHideDetail: function () { setOpenRev(null); },
