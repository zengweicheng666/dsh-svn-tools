...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        h("div", { className: "dsh-svn-sidessticky" },
          h("div", { className: "dsh-svn-sideshead" },
            ...
            h("div", { className: "dsh-svn-sidehead r", title: s.rightLabel ? "上一个版本" : "工作副本中的内容（右侧）" },
              (s.rightLabel || "工作副本") + " ▶", s.rightMissing ? "（已删除）" : "")
          ),
          navRow
        ),
        ...
        return h("div", { className: "dsh-svn-row", key: e.path, onClick: function () { props.onDiff(e.path); } },
          ...
          h("span", { className: "dsh-svn-rowops", onClick: function (ev) { ev.stopPropagation(); } },
            ...
            !isNew && !isConflict && e.status !== "!"
              ...
              : null,
            isNew
              ? h("button", { className: "dsh-svn-mini ok", onClick: function () { props.onAdd([e.path]); } }, "添加")
              : null,
            isNew
              ? h("button", { className: "dsh-svn-mini", onClick: function () { props.onIgnore(e.path); } }, "忽略")
              : null,
            !isNew && !isConflict
              ? h("button", { className: "dsh-svn-mini danger", onClick: function () { props.onDelete([e.path]); } }, "删除")
              : null,
            !isNew && !isConflict
              ? h("button", { className: "dsh-svn-mini", onClick: function () { props.onRevert([e.path]); } }, "还原")
              : null
          )
        );
      });
    ...
    function HistoryView(props) {
          ...
          h("div", { className: "dsh-svn-loghead" },
            ...
            props.onSwitchRev && !isCur
              ? h("button", { className: "dsh-svn-switchrev",
                  disabled: props.busy,
                  title: "切换工作副本到 r" + e.revision + "（svn update -r " + e.revision + "）",
      ...
      if (props.openRev !== null && props.openRev !== undefined) {
        ...
        if (entry) {
          ...
          var files = (entry.paths || []).map(function (p, idx) {
            ...
            var isChecked = multi && !!(props.checked && props.checked[p.path]);
            var isRunning = !!props.activePath && props.activePath === p.path;
            var cls = "dsh-svn-histfile" + (clickable ? "" : " disabled") +
              (isChecked ? " checked" : "") + (isRunning ? " running" : "");
            return h("div", { className: cls,
              key: p.path + "-" + idx,
              title: clickable ? "查看 r" + entry.revision + " 与上一版本的对比" : "不可对比（目录或不在当前工作副本内）",
              onClick: clickable ? function () { props.onOpenPath(entry.revision, p.path); } : null },
              multi && clickable
                ? h("input", { type: "checkbox", className: "dsh-svn-check",
                    checked: isChecked, disabled: props.busy,
                    title: isChecked ? "取消勾选" : "勾选此文件（批量执行撤销/回退/退回）",
                    onChange: function () { props.onToggleCheck(p.path); },
          ...
          // multi-select toolbar between the msg/files divider and the file
          // list: mode switch on the left, batch action buttons on the right
          // (only shown when files are checked and the set supports them).
          var checkedCount = 0;
          Object.keys(props.checked || {}).forEach(function (k) { if (props.checked[k]) checkedCount++; });
          var multiBar = null;
          if (hasFiles) {
            multiBar = h("div", { className: "dsh-svn-hisbar" },
              h("button", { className: "dsh-svn-mini" + (multi ? " on" : ""), disabled: props.busy,
                title: multi ? "切回单选模式（行悬停显示单文件按钮）" : "切换多选模式：文件行左侧出现勾选框，可批量执行撤销/回退/退回",
                onClick: props.onToggleMulti }, multi ? "多选 ✓" : "多选"),
              multi
          ...
          detail = h("div", { className: "dsh-svn-histdetail", ref: detailRef },
            ...
            h("div", { className: "dsh-svn-histdmsg", style: hasFiles ? { height: msgPct + "%" } : undefined }, entry.message || ""),
            hasFiles
            ...
            hasFiles ? h("div", { className: "dsh-svn-histdfiles" }, files) : null
          );
        }
      }
      ...
      // older page (30 entries) per click, down to the first revision; once
      // no older entry remains, the button becomes a "whole history shown"
      // note (only when more than one page was loaded).
      var listItems = rows;
      if (props.onLoadMore) {
        var footer = props.more
          ? h("div", { className: "dsh-svn-histmore" },
              h("button", { className: "dsh-svn-actbtn", disabled: props.busy,
                ...
                title: "再取 30 条更早的提交历史，可重复点击直到 r1" },
    ...
    function CommitView(props) {
      ...
      var canSubmit = props.busy !== true && props.msg.trim() !== "" && (commitAll || hasSel);
      var hint = commitAll
        ? "将提交全部 " + allCount + " 个变更"
        : hasSel
      ...
      return h("div", { className: "dsh-svn-commit" },
        ...
        h("div", { className: "dsh-svn-lbl", style: { color: !commitAll && !hasSel ? "#f0b429" : undefined } }, hint),
    ...
    function SvnPanel(props) {
      ...
      var [view, setView] = useState("commit");
      ...
      // history detail: multi-select mode + checked files + the file being
      // reverted right now (row highlight).
      var [multiMode, setMultiMode] = useState(false);
      ...
      function loadLog() {
        ...
        call("log", payload({ limit: 30, verbose: true })).then(function (v) {
          ...
          setLogMore(entries.length >= 30 && !!lastE && lastE.revision > 1);
          setView("history");
          loadHeadInfo();
        }).catch(function (e) { setErr(e.message || String(e)); })
          .finally(function () { setBusy(false); });
      ...
      // Append the next older page (up to 30 entries strictly below the
      // oldest revision shown) to the history list; the button disappears
      // once the server reports no more older entries remain.
      function loadMoreLog() {
        ...
        setBusy(true);
        setErr(null);
        call("log", payload({ limit: 30, verbose: true, olderThan: lastLog.revision })).then(function (v) {
          var next = v.entries || [];
          ...
          setLogMore(fresh.length >= 30 && last.revision > 1);
        }).catch(function (e) { setErr(e.message || String(e)); })
          .finally(function () { setBusy(false); });
      }
      ...
      function loadHeadInfo() {
        ...
        call("head-info", payload()).then(function (v) {
          ...
          setHeadInfo({ headRevision: null, error: e.message || String(e) });
      ...
      // list checkbox -> selection; checking a file switches the commit scope
      // to "selected only" (uncheck the commit-all toggle), clearing them all
      // switches back to commit-all.
      function updateSel(p, v) {
        var next = Object.assign({}, sel, { [p]: v });
        setSel(next);
        var any = Object.keys(next).some(function (k) { return next[k]; });
        setCommitAll(!any);
      }
      ...
      if (checkoutMode) {
        ...
        body = h(LocksView, { entries: locks || [], onUnlock: runUnlock });
      } else {
        ...
        body = h("div", { className: "dsh-svn-commitpage", ref: cmpWrapRef },
          h("div", { className: "dsh-svn-commitpage-list", style: { height: cmpPct + "%" } },
            h(ChangesView, { entries: entries || [], summary: summary, sel: sel, busy: busy,
              onDiff: showDiff, onToggle: updateSel,
          ...
          h("div", { className: "dsh-svn-commitpage-form" },
            h(CommitView, { entries: entries || [], sel: sel, msg: msg, busy: busy, commitAll: commitAll,
              aiBusy: aiBusy, onAiGenerate: generateMsg,
              onMsg: setMsg, onToggle: updateSel,
      ...
      return h("div", { className: "dsh-svn" },
        ...
        h("div", { className: "dsh-svn-toolbar" },
          h("div", { className: "dsh-svn-seg" },
            h("button", { className: "dsh-svn-tabbtn" + ((view === "blame" || view === "commit" || (view === "diff" && diff && !diff.fromHistory)) ? " on" : ""), onClick: function () { setView("commit"); } }, "提交"),
            h("button", { className: "dsh-svn-tabbtn" + ((view === "history" || (view === "diff" && diff && diff.fromHistory)) ? " on" : ""), onClick: function () { switchView("history"); } }, "历史"),
            h("button", { className: "dsh-svn-tabbtn" + (view === "locks" ? " on" : ""), onClick: function () { switchView("locks"); } }, "占用")
          ),
