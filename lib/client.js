...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ------------------------------------------------------------- entry
    // 'betterSidebar' stays in inject: the declared inject edge is what makes
    // the loader activate this bundle only once dsh-better-sidebar's service
    // exists (dropping it let this effect run before the provider was active,
    // and the tab silently never registered). A profile missing
    // dsh-better-sidebar surfaces the injected service as undefined here, so
    // the guard degrades with a loud console error instead of crashing.
