...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SidesView(props) {
      ...
      // The document changed (choose-left/right, external edit): the block
      // list is rebuilt, so the previous navigation index is meaningless —
      // reset it to "no current difference".
      useEffect(function () { setCurIdx(-1); }, [props.sides]);
    ...
    function SvnPanel(props) {
      ...
      // Keep the last loaded diff snapshot for change polling (the poll reads
      // a ref, so it never depends on stale closures of the state value).
      var diffRef = React.useRef(null);
      useEffect(function () { diffRef.current = diff; }, [diff]);

      // While the diff view is open, poll the file's sides every 3 s: when the
      // text changed (采用左侧/右侧 from this panel, agent svn tools, or any
      // external edit), silently refresh the diff view + the changes list.
      useEffect(function () {
        var path = view === "diff" && diff ? diff.path : null;
        if (!props.visible || !path) return;
        var timer = setInterval(function () {
          var snapshot = diffRef.current;
          if (!snapshot || snapshot.path !== path) return;
          call("diff-sides", payload({ path: snapshot.path })).then(function (sides) {
            var cur = diffRef.current;
            if (!cur || cur.path !== snapshot.path) return; // view switched away
            if (JSON.stringify(sides) === JSON.stringify(cur.sides)) return; // unchanged
            call("diff", payload({ path: cur.path })).then(function (res) {
              var next = { path: cur.path, text: res.diff, sides: sides };
              diffRef.current = next;
              setDiff(next);
              setNotice("检测到文件变化，已刷新");
            }).catch(function () { /* keep the previous view on error */ });
            loadAll(true); // refresh the changes list / summary silently
          }).catch(function () { /* transient svn error: ignore */ });
        }, 3000);
        return function () { clearInterval(timer); };
      }, [props.visible, view, diff, payload, loadAll]);
      ...
      function chooseDiff(block, mode) {
        ...
        call("diff-choose", payload({ path: diff.path, block: { start: start, end: end }, mode: mode })).then(function () {
          ...
          loadAll(true); // working copy changed → refresh the changes list too
