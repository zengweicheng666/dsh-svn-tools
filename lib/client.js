 ...
 * dsh-better-sidebar (ctx.get("betterSidebar").registerTab — read without
 * an inject requirement; a profile missing dsh-better-sidebar logs a clear
 * error instead of crashing) and renders a small
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CommitView(props) {
      ...
      var canSubmit = props.busy !== true && props.msg.trim() !== "" && allCount > 0 && (commitAll || hasSel);
      var canAi = props.aiBusy !== true && allCount > 0 && (commitAll || hasSel);
      var hint;
      if (allCount === 0) {
        hint = hiddenUnv > 0
      ...
      return h("div", { className: "dsh-svn-commit" },
        h("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
          ...
          h("button", { className: "dsh-svn-actbtn", disabled: !canAi, onClick: props.onAiGenerate, title: aiTitle },
    ...
    function SvnPanel(props) {
      ...
      // are excluded from the commit scope and the AI analysis too.
      var [showUnv, setShowUnv] = useState(true);
        ...
        body = h("div", { className: "dsh-svn-commitpage", ref: cmpWrapRef },
          ...
          h("div", { className: "dsh-svn-commitpage-form" },
            h(CommitView, { entries: listEntries, hiddenUnvCount: hiddenUnvCount, showUnv: showUnv,
              ...
              onMsg: setMsg,
              onCommitAll: updateCommitAll, onToggleShowUnv: setShowUnv, onCommit: runCommit }))
        );
      }
    ...
    // ------------------------------------------------------------- entry
    // dsh-better-sidebar is intentionally NOT injected: the betterSidebar
    // service only exists when that bundle is an active profile layer, and
    // this bundle must stay loadable without it. cordis `ctx.get(name)` reads
    // a service without the inject requirement and returns undefined when it
    // is absent, so a profile missing dsh-better-sidebar degrades with a loud
    // console error instead of a crashed (or silently parked) client bundle.
    var inject = ["slots", "sessions", "modules"];

    function apply(ctx) {
      ctx.effect(function () {
        var sidebar = ctx.get("betterSidebar");
        if (!sidebar) {
          console.error("[dsh-svn-tools] dsh-better-sidebar is not installed/enabled: the 'SVN' sidebar tab was not registered. Install it first: dsh plugin --profile web add dsh-better-sidebar (and keep it in dsh.profile.bundles, before dsh-svn-tools). The 33 svn_* agent tools are unaffected.");
          return;
        }
        ensureStyle();
