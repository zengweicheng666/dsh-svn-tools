  ...
  factory: (require) => {
    ...
    function SidesView(props) {
      ...
      var nav = function (delta) {
        ...
        // Scroll ONLY the inner .dsh-svn-sides container (scrollIntoView would
        // also scroll ancestor containers, shoving the nav row out of view).
        if (el && scroller && scroller.scrollTop !== undefined) {
          var cRect = scroller.getBoundingClientRect();
          var eRect = el.getBoundingClientRect();
          if (eRect.top < cRect.top || eRect.bottom > cRect.bottom) {
            var target = scroller.scrollTop + (eRect.top - cRect.top) - (cRect.height - eRect.height) / 2;
            try { scroller.scrollTo({ top: Math.max(0, target), behavior: "smooth" }); }
            catch (e2) { scroller.scrollTop = Math.max(0, target); }
          }
        }
      };
      var changeNo = -1;
