...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dsh-svn-histdivider{flex:0 0 auto;height:12px;cursor:row-resize;touch-action:none;display:flex;align-items:center;justify-content:center;background:rgba(148,163,184,.05);border-top:1px solid rgba(148,163,184,.22);border-bottom:1px solid rgba(148,163,184,.22)}",
      ".dsh-svn-histdivider::after{content:\"⇅\";font-size:11px;line-height:1;color:#8b949e;background:#161b22;border:1px solid rgba(148,163,184,.4);border-radius:4px;padding:1px 5px}",
      ".dsh-svn-histdivider:hover,.dsh-svn-histdivider:active{background:rgba(88,166,255,.16)}",
      ".dsh-svn-histdivider:hover::after,.dsh-svn-histdivider:active::after{color:#e6edf3;background:#1c2128;border-color:#58a6ff}",
      ".dsh-svn-histdetail{flex:1;min-height:0;overflow:auto;background:rgba(148,163,184,.07)}",
      ".dsh-svn-histdtop{position:sticky;top:0;z-index:3;background:#0d1117}",
      ".dsh-svn-histdhead{display:flex;gap:8px;align-items:baseline;padding:6px 12px 2px;font-size:11px;background:rgba(148,163,184,.08);border-bottom:1px solid rgba(148,163,184,.12)}",
      ".dsh-svn-histdmsg{padding:2px 12px 6px;font-size:12px;color:#c9d1d9;white-space:pre-wrap;word-break:break-all}",
      ".dsh-svn-histdfiles{padding:2px 0 6px}",
      ".dsh-svn-histfile{display:flex;align-items:center;gap:8px;padding:3px 12px;cursor:pointer;min-width:0}",
      ...
      ".dsh-svn-histfpath{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;color:#c9d1d9}",
    ...
    function HistoryView(props) {
      ...
      var rows = entries.map(function (e) {
        ...
        return h("div", { className: "dsh-svn-logrow" + (on ? " on" : ""), key: e.revision, onClick: function () { props.onSelectRev(e.revision); } },
      ...
      if (props.openRev !== null && props.openRev !== undefined) {
        ...
        if (entry) {
          ...
          detail = h("div", { className: "dsh-svn-histdetail" },
            h("div", { className: "dsh-svn-histdtop" },
              ...
              h("div", { className: "dsh-svn-histdmsg" }, entry.message || "")
            ),
            files.length > 0 ? h("div", { className: "dsh-svn-histdfiles" }, files) : null
          );
        }
      }
      return h("div", { className: "dsh-svn-hist", ref: wrapRef },
        h("div", { className: "dsh-svn-histlist", style: { height: topPct + "%" } }, rows),
        detail
      ...
      if (checkoutMode) {
        ...
        body = h(DiffView, { path: diff.path, diff: diff.text, sides: diff.sides,
          ...
          onBack: diff.fromHistory
            ...
            : function () { setDiff(null); setView("changes"); },
        ...
        body = h(HistoryView, { entries: logs || [], openRev: openRev,
          ...
          onSelectRev: function (r) { setOpenRev(openRev === r ? null : r); },
      ...
      return h("div", { className: "dsh-svn" },
        ...
        h("div", { className: "dsh-svn-toolbar" },
          h("div", { className: "dsh-svn-seg" },
            h("button", { className: "dsh-svn-tabbtn" + ((view === "changes" || view === "blame" || (view === "diff" && diff && !diff.fromHistory)) ? " on" : ""), onClick: function () { setView("changes"); } }, "变更"),
