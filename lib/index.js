...
function buildApi(ctx) {
  ...
  api.historyRevert = async (p) => {
    ...
    if (mode === 'to-this') {
      ...
      const { temp } = await catToTemp(cwd, url, revision);
      try {
        if (exists) {
          const same = await filesEqual(wcAbs, temp);
  ...
  api.log = async (p) => {
    ...
    let range = 'HEAD:1';
    if (p.olderThan !== undefined && p.olderThan !== null && p.olderThan !== '') {
      const bound = Number(p.olderThan);
      if (!Number.isInteger(bound) || bound <= 1) return { entries: [] };
      range = `${bound - 1}:1`;
    }
