 ...
 * NUL bytes. Resolves { temp, binary }; the caller owns `temp` and must
 * unlink it. The URL's non-ASCII segments must already be percent-encoded
...
async function catToTemp(cwd, url, rev) {
  ...
  let binary = false;
  const fh = await fs.open(temp, 'r');
  try {
    const head = Buffer.alloc(8192);
    const { bytesRead } = await fh.read(head, 0, head.length, 0);
    binary = head.subarray(0, bytesRead).includes(0);
  } finally {
    await fh.close();
  }
  return { temp, binary };
...
async function catUrl(cwd, url, rev) {
  const { temp, binary } = await catToTemp(cwd, url, rev);
  try {
...
function buildApi(ctx) {
   ...
   * only the fields the history page needs. */
  api.headInfo = async (p) => {
    ...
    // Actual local revision — svnversion reports the working copy's real
    // state (mixed-revision ranges like "752:754"), which svn info's base
    // revision under-reports for a partially-updated working copy.
    const local = await localRevisionOf(cwd);
    return {
  ...
  api.diffSidesRev = async (p) => {
    ...
    const { url } = await repoRelTarget(cwd, p.repoRel);
    const cat = async (rev) => {
  ...
  api.historyRevert = async (p) => {
    ...
    if (mode !== 'undo' && mode !== 'restore') {
      throw badRequest('mode must be "undo" or "restore"');
    }
    ...
    const existsAtRev = async (rev) => {
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        if (/non-existent|E160006|E200009|W170000|not found|No such|不存在/i.test(msg)) return false;
        throw error;
      }
    };
    ...
    const prevInfo = async () => {
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        if (/non-existent|E160006|E200009|W170000|not found|No such|不存在/i.test(msg)) return { exists: false, prevRev: null };
        throw error;
      }
    };
    ...
    const deleteAddedFile = async () => {
      ...
      const output = await runSvn(['delete', '--non-interactive', '--', wcAbs], { cwd, timeout: 120000 });
    ...
    const restoreContent = async (prevRev) => {
      ...
      const { temp } = await catToTemp(cwd, url, prevRev);
      try {
        if (exists) {
    ...
    /** The working copy's BASE last-changed revision of this file (local
     * svn info, no repository contact); null when it cannot be read (e.g.
  ...
  api.log = async (p) => {
    ...
    const argv = ['log', '--xml', '-r', 'HEAD:1', '-l', String(limit)];
